...
- **100 MB total size** — Captures exceeding this are rejected
- **500 resources max** — Pages with more than 500 assets are truncated
- **60s navigation timeout** — Page must load within 60 seconds
- **110s hard timeout** — Total capture must complete in 110 seconds

<Warning>
Very large or slow-loading pages may hit resource limits. The capture will complete with available assets, but some files may be skipped.
</Warning>
...
The server validates every URL:

1. **Hostname blocklist** — Rejects private IPs, localhost, internal domains
2. **DNS resolution check** — Verifies domain resolves to public IP
3. **Post-redirect re-check** — Validates final URL after redirects

### Data Handling

- **No storage** — ZIP is streamed directly to your browser, not stored on server
- **No logging** — Page content is not logged or persisted
- **Rate limit only** — Only capture timestamps are stored (for rate limiting)
- **No credentials** — Browser runs unauthenticated (public pages only)

## Common Use Cases

<CardGroup cols={2}>
  <Card title="Funnel Research" icon="chart-line">
    Download competitor landing pages to analyze copywriting, layout, and CTAs
  </Card>
  <Card title="Swipe File" icon="folder">
    Build an offline library of high-converting pages for creative inspiration
  </Card>
  <Card title="Page Archival" icon="archive">
    Preserve landing pages before they're taken down or changed
  </Card>
  <Card title="Template Customization" icon="palette">
...
### "Landing page capture is unavailable"

- **Cause:** Landing URL is a Meta platform (Facebook/Instagram)
- **Solution:** Page Ripper only works for external public pages. Meta URLs require login and cannot be captured.
